...
public:
    ListNode *mergeKLists(vector<ListNode *> &lists)
    {
        ...
        priority_queue<node, vector<node>, compare> heap; // Priority queue for storing k
                                                          // smallest elements pointed by right now.
        ListNode *head, *tail;                            // answer
        vector<ListNode *> ptr(k);                        // Storing and managing of list pointers.
        // Assigning the list pointers and building heap
        for (int i = 0; i < k; i++)
        {
            ptr[i] = lists[i];
            if (ptr[i])
        ...
        int index = heap.top().index;
        heap.pop();
        ptr[index] = ptr[index]->next; // updating the pointer
        if (ptr[index])
        { // Pushing newly pointed node if not null.
            heap.push(node(ptr[index], index));
        }
        // Make a list with rest of the nodes.
        while (!heap.empty())
        {
            ListNode *temp = heap.top().curr;
            index = heap.top().index;
            heap.pop(); // remove that element.
